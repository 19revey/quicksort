...
//  test
//
//  Created by YDuan on 3/23/16.
//  Copyright (c) 2016 Duan. All rights reserved.
//
...
void find_neighbor_spatial(int n,double *xp,double *yp,double dis,int **neighborListoutput,int MAX_LIST)
{
    int numParticles=n;
    ...
    float3 worldOrigin;
    worldOrigin = make_float3(0.0f, 0.0f, 0.0f);
 ...
 //   uint maxParticlesPerCell;
  ...
  //  uint  *m_hCellEnd;
    ...
    */
    ...
    uint tempList[numParticles][MAX_LISTELEM];
    memcpy(neighborList,h_neighborList,numParticles*MAX_LISTELEM*sizeof(uint));
    memcpy(tempList,neighborList,numParticles*MAX_LISTELEM*sizeof(uint));
    for (int i=0; i<numParticles; i++) {
        for (int j=0; j<MAX_LISTELEM; j++) {
            neighborList[m_hGridParticleIndex[i]][j]=tempList[i][j];
        }
    }
    memcpy(neighborListoutput,neighborList,numParticles*MAX_LISTELEM*sizeof(uint));
    ...
    for(int i=0; i<numParticles; i++)
    {
        printf("%.4d,   %.4d,      %.4d,   %.4d,   %.4d,   %.4d,      %.4d\n",i,m_hGridParticleHash[i], neighborList[i][0],neighborList[i][1],neighborList[i][2],neighborList[i][3],neighborList[i][MAX_LISTELEM-1]);
    }
